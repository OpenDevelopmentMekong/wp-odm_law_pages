...
if (!class_exists('Odm_Tabular_Pages_Post_Type')) {
    ...
    class Odm_Tabular_Pages_Post_Type
    {
        ...
        {
            $labels = array(
            'name' => __('Database', 'post type general name', 'odi'),
            'singular_name' => __('Database', 'post type singular name', 'odi'),
            'menu_name' => __('Database', 'admin menu for tabular pages', 'odi'),
            'name_admin_bar' => __('Database', 'add new on admin bar', 'odi'),
            'add_new' => __('Add new', 'tabular page', 'odi'),
            'add_new_item' => __('Add new database', 'odi'),
            'new_item' => __('New database', 'odi'),
            'edit_item' => __('Edit database', 'odi'),
            'view_item' => __('View database', 'odi'),
            'all_items' => __('All databases', 'odi'),
            'search_items' => __('Search databases', 'odi'),
            'parent_item_colon' => __('Parent databases:', 'odi'),
            'not_found' => __('No database found.', 'odi'),
            'not_found_in_trash' => __('No database found in trash.', 'odi'),
            );
				...
				public function tabular_options_box($post = false)
	      {
            ...
            <div id="multiple-site">
	          ...
	          <div id="tabular_options_box">
	            <h4><?php _e('Choose dataset type', 'odi');?></h4>
	            <select id="_attributes_dataset_type" name="_attributes_dataset_type[]" multiple="multiple">
                <option value="all" <?php if(!empty($dataset_type) && in_array("all", $dataset_type)): echo "selected"; endif;  ?>><?php _e('All', 'odi');?></option>
	              <option value="dataset" <?php if(!empty($dataset_type) && in_array("dataset", $dataset_type)): echo "selected";  endif; ?>><?php _e('Dataset', 'odi');?></option>
	              <option value="library_record" <?php if(!empty($dataset_type) && in_array("library_record", $dataset_type)): echo "selected=selected"; endif;?>><?php _e('Publication', 'odi');?></option>
	              <option value="laws_record" <?php if(!empty($dataset_type) && in_array("laws_record", $dataset_type)): echo "selected=selected"; endif;?>><?php _e('Laws record', 'odi');?></option>
	              <option value="agreement" <?php if(!empty($dataset_type) && in_array("agreement", $dataset_type)): echo "selected=selected"; endif; ?>><?php _e('Agreement', 'odi');?></option>
	            </select>

              <div class="language_settings language-en">
                <h4><?php _e('The attributes of the Dataset that would like to display', 'odi'); ?></h4>
  							...
  							<p class="description"><?php _e('Please specify the attributes plus their labels, separated by => and line breaks. Multiple attributes can be used for a label, separated by comma. eg. odm_document_type,odm_agreement_document_type => Document type', 'odi'); ?></p>

  							<h4><?php _e('Id to Label mapping for values', 'odi'); ?></h4>
  							...
  							<p class="description"><?php _e('Please specify the ids plus their labels, separated by => and line breaks', 'odi'); ?></p>

  						  <h4><?php _e('Column ids linking to detail page', 'odi');?></h4>
                ...
                <h4><?php _e('Group Data in Column', 'odi');?></h4>
  			        ...
  			        <p class="description"><?php _e('Eg. To group records by a certain column. please specify the index, not the name.', 'odi'); ?></p>
              </div>
              ...
              <?php if (odm_language_manager()->get_the_language_by_site() != "English") { ?>
                <div class="language_settings language-localization">
                  <h4><?php _e('The attributes of the Dataset that would like to display', 'odi'); ?></h4>
    							...
    							<p class="description"><?php _e('Please specify the attributes plus their labels, separated by => and line breaks', 'odi'); ?></p>
        ...
        public function tabular_filters_box($post = false)
          ...
          <div id="tabular_filters_box">
            <h4><?php _e('List of default filters', 'odi'); ?></h4>
            <p class="description"><?php _e('Please specify which of the default filters are displayed', 'odi'); ?></p>
            ...
            <h4><?php _e('List of additional filters with Resource ID of CKAN', 'odi'); ?></h4>
                ...
                <p class="description"><?php _e('Please specify the field names and the corresponding resource ids, separated by => and line breaks', 'odi'); ?>  <br />eg. odm_document_type  =>  <a href="https://data.opendevelopmentmekong.net/dataset/dat/resource/09f75141-0885-44f7-bcfc-8cd1e3779ff5?type=dataset" target="_blank">09f75141-0885-44f7-bcfc-8cd1e3779ff5</a></p>

            <h4><?php _e('Custom list of element filters', 'odi'); ?></h4>
                ...
                <p class="description"><?php _e('Please specify the elements (ids/attributes) and the corresponding field name, separated by line breaks. Please blank it, if the filter was created with Resource ID of CKAN.', 'odi'); ?>  <br />eg. international_treaty</p>
                ...
                <div id="group_filter_setting" class="filter_setting hide">
                  <div id="multiple-site">
                    <p>
                      ...
                      <label for="en-filter-group"><?php _e('ENGLISH', 'odi');?></label> &nbsp;
                      <?php if (odm_language_manager()->get_the_language_by_site() != "English"): ?>
                        ...
                        <label for="localization-filter-group"><?php _e(odm_language_manager()->get_the_language_by_site(), 'odi');?></label>
                      <?php endif; ?>
                    </p>
                  </div>
                  <div class="language_settings language-en">
                    ...
                    <p class="description"><?php _e('Please list the ids (attributes) that are avialbe in the field name separated by commands and group them separated by line breaks', 'odi'); ?></p>
            ...
            <h4><?php _e('List of additional filters with types', 'odi'); ?></h4>
            ...
            <p class="description"><?php _e('Please specify the field names and the corresponding field type, separated by => and line breaks.', 'odi'); ?></p>

            <h4><?php _e('Create Select Filter by Column Index', 'odi'); ?></h4>
            ...
            <p class="description"><?php _e('Filter selectors will create automatically by adding the column index and separated by comma.', 'odi'); ?></p>
          </div>
